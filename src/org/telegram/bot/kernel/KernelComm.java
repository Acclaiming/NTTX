...
import org.telegram.api.engine.file.*;
import org.telegram.api.engine.storage.*;
...
public class KernelComm implements IKernelComm {
    ...
    public <T extends TLObject> T doRpcCallSync(final TLMethod<T> method) throws ExecutionException, RpcException {
        T answer = null;
        if (getApi() != null) {
            final Future<T> result = this.exe.submit(new Callable<T>(){
				
				public T call () throws RpcException {
    ...
    public <T extends TLObject> T doRpcCallSyncNoAuth(final TLMethod<T> method) throws ExecutionException {
        ...
        final Future<T> result = this.exe.submit(new Callable<T> () {
            ...
            try {
                answerInternal = api.doRpcCallNonAuth(method);
            } catch (RpcException e) {
    ...
    @Override
    public <T extends TLObject> void doRpcCallAsync(final TLMethod<T> method, final TelegramFunctionCallback<T> callback) {
        if (getApi() != null) {
            final CompletableFuture<T> result = CompletableFuture.supplyAsync(new Supplier<T>() {
					public T get() {
						T internalAnswer = null;
						try {
							internalAnswer = getApi().doRpcCall(method);
            ...
            result.handleAsync(new BiFunction<T,Throwable,Object>() {

					@Override
					public Object apply(T ok,Throwable ex) {
						if (ok != null) {
    ...
    public void doRpcCallAsyncNoReturn(final TLMethod<TLObject> method) {
        this.exe.submit(new Runnable() {
			...
			public void run() {
            try {
                final TLObject answer = api.doRpcCall(method);
                handleAffectedMessagesAndHistory(answer);
            } catch (RpcException e) {

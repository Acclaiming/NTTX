        ...
        if (updates instanceof TLUpdateShortMessage) {
            ...
            if (correctSeq) {
				...
				for (TLAbsUpdate update : tlUpdates.getUpdates()) {
                    ...
                    final UpdateWrapper wrapper = new UpdateWrapper(update);
                    wrapper.setParams(update.getPts(), update.getPtsCount(), tlUpdates.getDate(), tlUpdates.getSeq(), 0);
                    if (disablePtsCheck) {
            ...
            if (correctSeq) {
                ...
                for (TLAbsUpdate update : updatesCombined.getUpdates()) {

                    final UpdateWrapper wrapper = new UpdateWrapper(update);
                    wrapper.setParams(update.getPts(), update.getPtsCount(), updatesCombined.getDate(), updatesCombined.getSeq(), updatesCombined.getSeqStart());
                    if (disablePtsCheck) {
                        wrapper.disablePtsCheck();
                    }
                    updateHandlerThread.addUpdate(wrapper);

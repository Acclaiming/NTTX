...
public class YouTubeExtractor {
    ...
    public LinkedList<YtFile> execute(String ytUrl) {
        videoID = null;
        if (ytUrl == null) {
            return null;
        }
        ...
        if (videoID != null) {
            try {
                return new LinkedList<>(getStreamUrls().values());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private HashMap<Integer,YtFile> getStreamUrls() throws IOException, InterruptedException {
        ...
        for (String encStream : streams) {
            ...
            if (curJsFileName != null) {
                ...
                if (mat.find()) {
                    encSignatures.put(itag, URLDecoder.decode(mat.group(2), "UTF-8"));
                } else {
        ...
        if (encSignatures != null) {
            ...
            if (signature == null) {
				...
				Integer[] keys = encSignatures.keySet().toArray(new Integer[encSignatures.size()]);

                for (int i = 0; i < keys.length && i < sigs.length; i++) {
                    int key = keys[i];
                    if (key == 0) {
    ...
    private void parseDashManifest(String dashMpdUrl, HashMap<Integer,YtFile> ytFiles) throws IOException {
    ...
    private void readDecipherFunctFromCache() {
        File cacheFile = new File(Env.CACHE_DIR ,"ytb_cache/" + CACHE_FILE_NAME);
    ...
    private void decipherViaWebView(final HashMap<Integer,String> encSignatures) {
		...
		decipheredSignature = ScriptUtil.eval(stb.toString()).toString();

...
public class TieBaApi {
		...
		try {
			...
			switch(statusCode) {  
			    ...
			    case "4":
			    	//用户名或密码错误
			    	map.put("status", -3);
					map.put("message", "用户名密码错误，请重新输入密码，或使用 /cancel 取消 (");
					break;  
			    case "257":
			    	//请输入验证码
	...
	public Map<String, Object> oneBtnToSign(String bduss, String stoken){
		...
		for (MyTB tb : list) {
			...
			results.add(this.signOneTieBa(tb.getTbName(), tb.getFid(), bduss, tbs));
		...
		for (Map<String,Object> result : results) {
			...
			} else if (result.get("error_code").toString().equals("160002")) {
	...
	public void getTbsByUid(String uid, List<Map<String, Object>> tiebas, int page, Integer curpn) {
		...
		if (!(JsonKit.getInfo("forum_list", tStr) instanceof com.alibaba.fastjson.JSONArray)) {
			...
			if(lj3 != null) {
				...
				for (Map<String,Object> item : lj3) {
					
					item.put("type", "non-gconforum");
			...
			if(lj4!=null) {
				...
				for (Map<String,Object> item : lj4) {
	...
	public Map<String, Object> oneBtnToSign(String bduss) {
		...
		for (Map<String,Object> tb : list) {
		
			results.add(this.signOneTieBa(tb.get("name").toString(), Integer.parseInt(tb.get("id").toString()), bduss, tbs));
		...
		msg.put("已签到", signedCount);
		msg.put("签到失败", (totalCount - signedCount - signCount));
		msg.put("耗时", (System.currentTimeMillis()-start) / 1000 +"s");

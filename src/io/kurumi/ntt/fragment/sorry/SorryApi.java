...
public class SorryApi {
    ...
    public static LinkedHashMap<String, SorryApi> templates;
    ...
    private static File make(String template, String[] lines) {
        ...
        File templateDir = FileUtil.file(Env.ROOT_DIR, "res/templates/" + template);
        ...
        if (mp4File.isFile()) {
            ...
            if (NumberUtil.isNumber(template)) {
                ...
                String url = "https://app.xuty.tk/memeet/api/v1/template/make";

                JSONObject query = new JSONObject();

                query.put("id", template);
                ...
                JSONObject gifObj = new JSONObject(result.body());

                if (!gifObj.getBool("ok", false)) return null;

                String gif = gifObj.getByPath("body.url", String.class);

                HttpUtil.downloadFile(origin + gif, cache);

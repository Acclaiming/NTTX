...
public class FFMpeg {
	...
	public static boolean makeGif(File globalPalettePicPath,File template,File ass, File out) {
        ...
        try {
            ...
            return RuntimeUtil.exec("ffmpeg -i " + template + " -i " + globalPalettePicPath.getPath() + " -lavfi -vf ass=" + ass.getPath() + ",fps=50,scale=480:-1:flags=lanczos[x];[x][1:v]paletteuse -y " + out.getPath()).waitFor() == 0;

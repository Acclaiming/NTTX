...
public class FFMpeg {
	...
	public static boolean toGif(File globalPalettePicPath,File in,File out) {
		...
		try {
			...
			return RuntimeUtil.exec("ffmpeg -i " + in.getPath() + " -i " + globalPalettePicPath.getPath() + " -b 2048k -r 20 -lavfi fps=15,scale=270:-1:flags=lanczos[x];[x][1:v]paletteuse -y " + out.getPath()).waitFor() == 0;

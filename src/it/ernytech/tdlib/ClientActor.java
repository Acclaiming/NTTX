    ...
    private void run() {
        ...
        new Thread(new Runnable() {
				...
				public void run() {
					...
					while (!executionLock.isLocked()) {

						List<Response> responseList = client.receive(timeout,1000);
						...
						for (Response response : responseList) {
							...
							if (!TdApi.Error.class.isInstance(response.getObject())) {
								...
								tdCallback.getReceiveCallback().onResult(response);

							} else {

								tdCallback.getErrorCallback().onError(response);
							}
